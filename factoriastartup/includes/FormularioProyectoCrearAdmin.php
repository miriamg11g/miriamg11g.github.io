...
class FormularioProyectoCrearAdmin extends Formulario
{
    public function __construct()
    {
        parent::__construct('formProyecto', ['urlRedireccion' => 'ProyectosMostrarAdmin.php', 'enctype'=> 'multipart/form-data']);
    }
    ...
    protected function generaCamposFormulario(&$datos)
    {
        ...
        $erroresCampos = self::generaErroresCampos(['nombre', 'sector', 'short_description', 'description', 'needs', 'img', 'usuario'], $this->errores, 'span', array('class' => 'error'));
        ...
        $html = <<<EOF
            ...
            <div class="formulario">                          
                    ...
                    <input type="text" name="usuario" required="obligatorio" value="$usuario" />
    ...
    protected function procesaFormulario(&$datos){
        ...
        $id = trim($datos['usuario'] ?? '');
        $idUsuario= Usuario::buscaIdUsuario($id);
        if ( empty($idUsuario) || $idUsuario == null) {
            $this->errores['usuario'] = "El usuario no existe en la BBDD";
        }
